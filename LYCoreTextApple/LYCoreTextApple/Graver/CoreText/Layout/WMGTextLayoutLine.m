...
@property (nonatomic,strong) NSArray *strikeThroughFrames;
@end
...
- (id)initWithCTLine:(CTLineRef)lineRef truncatedLine:(CTLineRef)truncatedLineRef baselineOrigin:(CGPoint)baselineOrigin textLayout:(WMGTextLayout *)textLayout {
    if (self = [super init]) {
        if (lineRef) {
            ...
            CFRange range = CTLineGetStringRange(lineRef); //(0, 6)
            ...
            if (textLayout.baselineFontMetrics.leading != NSNotFound && textLayout.baselineFontMetrics.leading) {
            ...
            [textLayout.attributeString enumerateAttribute:WMGTextStrikethroughStyleAttributeName inRange:_originStringRange options:0 usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
                if (value) {
...
-(void)enumerateRunsUsingBlock:(void (^)(NSUInteger, NSDictionary * _Nonnull, NSRange, BOOL * _Nonnull))block {
    if (!_lineRef || !block) {
        return;
    }
